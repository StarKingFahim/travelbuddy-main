...
const MessageBox = ({ chatId }) => {
	...
	return (
		<>
			<div
				className="py-6 sm:px-6 px-3 w-full h-[7vh] font-semibold flex justify-between items-center bg-green-800 text-white cursor-pointer"
			...
			{isChatDetailsBox && (
				<div
					className={`h-[60vh] w-full max-w-96 absolute top-0 left-0 z-20 p-1 ${
						isExiting ? "exit-block" : "enter-block"
					}`}
				>
					<div
						ref={chatDetailsBox}
						className="flex border border-green-400 bg-green-800 overflow-hidden rounded-lg"
					>
						<ChatDetailsBox />
					</div>
				</div>

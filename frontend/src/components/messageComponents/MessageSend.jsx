...
const MessageSend = ({ chatId }) => {
	...
	return (
		<>
			{/* {mediaBox && (
                <div className="border-green-500 border rounded-md absolute bottom-[7vh] mb-1 left-2 bg-green-800 w-60 h-48 ">
                    <img
                        src={mediaURL}
                        alt="media"
                    ...
                    <MdOutlineClose
                        title="Delete"
                        size={25}
                        className="absolute top-2 right-3 cursor-pointer text-white bg-green-800 rounded-xl p-1"
                        onClick={clearMediaFile}
                    />
                </div>
            )} */}
			<form
				className="w-full flex items-center gap-1 h-[7vh] p-3 bg-green-800 text-white"
			...
			>
				...
				<span className="flex justify-center items-center">
					{newMessage?.trim() && !isSendLoading && (
						<button
							className="outline-none p-2 border-green-500 border-l"
					...
					{isSendLoading && (
						<button className="outline-none p-2 border-green-500 border-l">
							<LuLoader
								title="loading..."
								fontSize={18}

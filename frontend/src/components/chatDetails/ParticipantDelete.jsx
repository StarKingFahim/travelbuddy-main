...
const ParticipantDelete  = ({ setMemberAddBox }) => {
	...
	return (
		<div className="p-2">
			{selectedChat?.groupAdmin?._id === authUserId && (
				<div
					className="w-full my-2 h-12 border-green-500 border rounded-lg flex justify-start items-center p-2 font-semibold gap-2 hover:bg-black/50 transition-all cursor-pointer text-white"
			...
			<div className="min-h-0.5 w-full bg-green-900/50"></div>
			<div className="flex flex-col items-start w-full justify-center gap-1 mb-3 mt-3">
				{selectedChat?.users?.map((user) => {
					return (
						<div
							key={user?._id}
							className="w-full h-12 border-green-500 border rounded-lg flex justify-start items-center p-2 font-semibold gap-2 transition-all cursor-pointer text-white"
						>
							<img

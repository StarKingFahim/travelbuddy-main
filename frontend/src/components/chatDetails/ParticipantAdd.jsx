...
import { ChatLoadingSmall } from "../loading/ChatLoading";
import { IoCheckmarkCircleOutline, IoPersonAddOutline } from "react-icons/io5";
import { VscError } from "react-icons/vsc";

const ParticipantAdd = ({ setMemberAddBox }) => {
	...
	return (
		<div className="p-2">
			...
			<div className="min-h-0.5 w-full bg-green-900/50 mb-2"></div>
			<div
			...
			<div className="w-full flex flex-nowrap items-center justify-center gap-2 mb-2">
				<input
					...
					className="w-2/3 border border-green-600 py-1 px-2 font-normal outline-none rounded-md cursor-pointer bg-transparent active:bg-black/20"
					...
					<>
						...
						{selectedUsers?.map((user) => {
							return (
								<div
									key={user?._id}
									className="w-full h-12 border-green-500 border rounded-lg flex justify-start items-center p-2 font-semibold gap-2 hover:bg-black/50 transition-all cursor-pointer text-white"
								>
									<img
									...
									<div
										title="Add User"
										className="border border-green-600 p-2 w-fit font-normal outline-none rounded-md cursor-pointer bg-transparent active:bg-black/20 hover:bg-black/50"
										onClick={() =>
											handleAddUser(
												user?._id,
												user?.firstName
			...
			{addUserName && (
				<>
					...
					<div className="px-2 w-full fixed bottom-1 right-0">
						<div className="w-full h-12 border-green-500 bg-blue-950 border rounded-lg flex justify-between items-center p-2 font-semibold gap-2 transition-all cursor-pointer text-white ">
							...
							<div className="flex gap-1">
								<div
									...
									className="border border-green-600 p-1.5 w-fit font-normal outline-none rounded-md cursor-pointer bg-transparent active:bg-black/20"
								>
